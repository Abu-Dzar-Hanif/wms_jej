...
class WarehouseController extends Controller
{
    ...
    {
        return view('pages.editor.warehouse.index');
    }
    ...
    public function getData(Request $request):JsonResponse
    {
        ...
        try {
            $query = Warehouse::where('name', 'LIKE', '%'.$cari.'%');
            $wh_total = $query->count();
            $wh = $query->offset($start)
                ->limit($limit)
                ->get();
            $data['draw'] = intval($request->input('draw'));
            $data['recordsTotal'] = $wh_total;
            $data['recordsFiltered'] = $wh_total;
            $data['data'] = $wh;
            $data['success']=1;
    ...
    public function StoreUpdateData(Request $request):JsonResponse
    {
        ...
        try {
            $rules = [
                ...
                'code' => 'required|string|max:255|unique:warehouse,code,'. $request->id,
                'type' => 'required|integer',
            ];
            $massages = [
                'required' => ':attribute wajib diisi',
                'code.unique' => 'warehouse sudah ada',
            ...
            if ($validator->fails()){
                ...
                Warehouse::updateOrCreate(['id' => $validData['id']],$data);
